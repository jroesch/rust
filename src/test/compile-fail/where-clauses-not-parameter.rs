...
//~^ cannot bound type `core::option::Option<int>`, where clause bounds may
...
trait Baz<T> where int : Eq {
    //~^ ERROR cannot bound type `int`, where clause bounds may only
    fn baz() where String : Eq; //~ ERROR cannot bound type `collections::string::String`
    //~^ ERROR cannot bound type `int`, where clause
}
